...
class MonitoringController extends Controller
{
    ...
    public function activity()
    {
        ...
        if (Schema::hasTable('audit_logs')) {
            $auditQuery->with('actor');
            if (Schema::hasColumn('audit_logs', 'occurred_at')) {
                $auditQuery->latest('occurred_at');
            } else {
                $auditQuery->latest();
            }
        }
    ...
    public function users(Request $request)
    {
        $request->validate([
            ...
            'sort' => 'nullable|string|in:created_at,last_seen_at',
        ]);
        ...
        $users = User::query()
            ...
            ->when(
                $request->input('sort') === 'created_at' || !Schema::hasColumn('users', 'last_seen_at'),
